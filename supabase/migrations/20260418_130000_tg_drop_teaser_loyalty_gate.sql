...
update public.tg_drop_teasers
set published_at = coalesce(published_at, updated_at, created_at)
where published_at is null;

create index if not exists tg_drop_teasers_active_published_idx
  on public.tg_drop_teasers(is_active, published_at desc);
...
begin
  ...
  if not (v_has_early_access or v_is_open_for_all) then
    return null;
  end if;
...
grant execute on function public.tg_get_active_drop_teaser_for_user(bigint) to anon, authenticated;

revoke select on public.tg_drop_teasers from anon, authenticated;
