alter table public.tg_orders
  add column if not exists delivery_base_fee_rub integer null,
  add column if not exists delivery_markup_rub integer null,
  add column if not exists delivery_total_fee_rub integer null;
...
begin
  ...
  if v_delivery_markup_rub <> 60 then
    raise exception 'CHECKOUT_DELIVERY_MARKUP_INVALID';
  end if;
