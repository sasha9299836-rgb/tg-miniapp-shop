...
import { syncShipmentStatusForOrder } from "../_shared/cdekShipment.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(JSON.stringify({ scope: "shipment", event: "shipment_status_sync_request", orderId }));
    const result = await syncShipmentStatusForOrder(supabase, cdekProxyBaseUrl, orderId);
    return json(result);
  } catch (error) {
